...
///
/// The lists below are the kinds `append_event(` actually writes (grepped across
/// `src/`), not a guess. A first pass guessed at plausible kind names — things
/// like `agent_online`, `file_put`, `message_delivered`, `room_deleted`,
/// `agent_renamed`, a bare `joined`/`left` — and none of those literal strings
/// are ever emitted. The real, verified set is eleven kinds; see the report for
...
const FAMILIES: [KindTone, string[]][] = [
  ...
  // into being.
  ['human', ['agent_registered', 'room_joined']],
